...
class Gauss:
        ...
        for i in range(n):
            ...
            if self.codebus[i+1]==1: #PQ bus
                ...
                if self.q[i] + ((self.qload.get(i+1, 0)) or 0) / self.sbase <= self.qmin[i+1] / self.sbase:
                    print('hehehehehehehehhehehehehehehhhehehhe')
    ...
    def cal_gaus(self):
        ...
        while True:
            ...
            if max_dv <= accur or k == 500:
                break
        self.k_dict = dict([('Tổng số vòng lặp', k)])
        self.v = []
        self.v_delta = []
    ...
    def cal_gaus1(self):
        ...
        for i in range(len(bus_result['NO'])):
            self.v[i] = complex(float(bus_result['MAG'][i]), float(bus_result['ANG'][i])*(cmath.pi/180))
        # chạy vòng lặp
        k = 0
        while True:
            k += 1
            ...
            if max_dv <= accur or k == 500:
                break
        self.k_dict = dict([('Số bước lặp', k)])
        self.v = []
        self.v_delta = []
        for i in range(len(vn)):
            self.v.append(abs(vn[i]))
